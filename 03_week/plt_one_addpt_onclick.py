...
#   @output.capture()  # debug
    ...
    def resize_sq(self, bcid):
        ...
        # 1. Try to find rectangles (handles multiple Matplotlib versions)
        rects = getattr(bcid, 'rects', getattr(bcid, 'rectangles', None))
        
        # 2. If None, search the Axes for the drawn patches (Modern Matplotlib fix)
        if rects is None:
            import matplotlib.patches as patches
            rects = [p for p in bcid.ax.patches if isinstance(p, patches.Rectangle)]

        # 3. Only proceed if we actually found rectangles to avoid 'NoneType' error
        if rects and len(rects) > 0:
            rect = rects[0]
            h = rect.get_height()
            rect.set_height(3 * h)

            # Update coordinates for the 'X' lines
            bbox = rect.get_bbox()
            ymax, ymin = bbox.y1, bbox.y0

            # 4. Handle the 'X' lines inside the box
            lines = getattr(bcid, 'lines', None)
            if lines and len(lines) > 0:
                # lines[0] contains the two Line2D objects for the first checkbox
                line1, line2 = lines[0]
                line1.set_ydata([ymax, ymin])
                line2.set_ydata([ymin, ymax])
        
        # 5. Refresh the canvas to show changes
        if hasattr(bcid.ax.figure.canvas, 'draw_idle'):
            bcid.ax.figure.canvas.draw_idle()